...
def find_on_right_click(pos):
    try:
        ...
        if pos[1] != 225:
            base_cord1[1] -= round((base_cord_change * scale_modifier * 1.36) * ((pos[1] - 225) / 225), 8)
        toponym_longitude, toponym_lattitude = base_cord1

        search_api_server = "https://search-maps.yandex.ru/v1/"
        api_key = "dda3ddba-c9ea-4ead-9010-f43fbc15c6e3"
        ot = find_address(','.join(map(lambda x: str(x), base_cord1)))
        ...
        buttons[-1].txt = org_name + ', ' + org_address
        print(org_name + ', ' + org_address)
        # Получаем координаты ответа.
        point = organization["geometry"]["coordinates"]
        if ((point[0] - base_cord[0]) ** 2 + (point[1] - base_cord[1]) ** 2) ** 0.5 <= 50:
            is_map_point = True
            find_address(','.join(map(lambda x: str(x), point)))
            base_cord = point[:]
            map_point = base_cord[:]
    except Exception:
        pass
